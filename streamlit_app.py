...
def build_mapping_ui(wb, raw_df: pd.DataFrame):
    ...
    with center:
        ...
        for th in template_headers:
            ...
            def _make_cb(nk=norm):
                def _cb():
                    _register_edit(nk)  # record normalized template key, not the widget key
                return _cb
    ...
    with right:
        ...
        if st.button("Auto‑map (exact)"):
            ...
            for th in template_headers:
                ...
                cand = raw_norm_to_originals.get(tnorm)
                if cand:
                    st.session_state[key] = cand[0]  # first matching raw header
                    _register_edit(tnorm)
                    filled += 1
