...
def read_any_dataframe(uploaded_file) -> pd.DataFrame:
    """Read CSV/XLS/XLSX into DataFrame with object dtype to preserve text."""
...
def infer_mapping(df_map: pd.DataFrame) -> Dict[str, str]:
    """
    Build mapping {template_header -> raw_header}.
    The mapping file must have at least two columns:
    - First column = RAW
    - Second column = TEMPLATE
    """
    ...
    raw_col, template_col = None, None
    if len(cols) >= 2:
...
def process_workbook(wb, raw_df: pd.DataFrame, mapping: Dict[str, str]) -> Tuple[int, List[str]]:
    """
    Apply mapping into the FIRST sheet only.
    - Mapping uses row-2 headers.
    - Data filling begins from ROW 3.
    - Row 2 is header row (untouched).
    - Defaults in row 3 are preserved and copied down.
    ...
    protected_cols, defaults_present_cols = set(), set()
    for c in range(1, max_col + 1):
    ...
    # Validate mapping headers exist in row-2
    ...
    for i in range(n_rows):
        ...
        if target_row > 3:
            for c in defaults_present_cols:
                copy_default_cell_to_row(ws, 3, target_row, c)
...
# ----------------------------
# INPUT SECTIONS (ordered)
# ----------------------------
st.subheader("1) Upload Raw sheet (CSV/XLSX)")
raw_file = st.file_uploader("Upload Raw sheet", type=ACCEPTED_RAW_TYPES, key="raw_upload")

st.subheader("2) Upload eBay US Template (XLS/XLSX)")
...
with st.expander("Mapping file guidance", expanded=False):
    st.markdown(
        ...
        **Example CSV:**
        ...
        - Data writing begins at **row 3**.  
        - Any **defaults in row 3** are **preserved** and copied down.  
        """
    )
...
# PROCESS
# ----------------------------
if raw_file and template_file and mapping_file:
    try:
        ...
        wb = load_workbook(filename=io.BytesIO(template_bytes), data_only=False)
    ...
    st.info("Please upload the **Raw sheet**, **Template**, and **Mapping file** to begin.")
