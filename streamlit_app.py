...
def get_template_headers(ws: Worksheet) -> Tuple[Dict[int, str], Dict[str, int], int]:
    ...
    for c in range(1, max_col + 1):
        ...
        if v is not None and str(v).strip() != "":
            ...
            by_name[normalize(header_text)] = c
...
def highlight_duplicates(ws: Worksheet, keywords: Tuple[str, ...] = ("sku", "mpn", "upc")) -> Dict[str, int]:
    ...
    # Resolve target columns by keyword-in-header (case-insensitive)
    targets = []
...
if template_file and raw_file and mapping_file:
    try:
        ...
        # 3b) Highlight duplicates in SKU/MPN/UPC columns
        dup_summary = highlight_duplicates(wb.worksheets[0])
        if dup_summary:
